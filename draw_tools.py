...
def project_board(output_xml, board2cam_rvec, board2cam_tvec, cam_intrinsic, cam_distort):
    check_rvec(board2cam_rvec)
    check_tvec(board2cam_tvec)
    
    board_points = np.array(create_board_points())

    cam_points, _ = cv2.projectPoints(board_points,
        board2cam_rvec,
        board2cam_tvec,
        cam_intrinsic,
        cam_distort
    ...
    cam_points = np.array(cam_points).squeeze()

    print(board_points)
    print(cam_points)
    file = cv2.FileStorage(output_xml, cv2.FileStorage_WRITE)
    file.write('cam_points', cam_points)
    file.release
